...
def block_average(mhw, dstime=None, period=None, blockLength=1, mtime='time_start',
    ...
    if sw_temp:
        if sw_cats:
            print("Both ts and climatologies are available, " \
...
def check_variables(dstime):
    ...
    if 'ts' not in variables:
            sw_temp = False
            print("We cannot identify which variable is the temperature timeseries as it is not named 'ts'")
...
def find_across(mhw):
    ...
    mhw_span = mhw.where(mhw['time_start'].dt.year != 
                         mhw['time_end'].dt.year).dropna(dim='events')
    return mhw_span
...
def mhw_rank(mhwds):
    ...
    variables =  [k for k in mhwds.keys() if not any(x in k for x in
                  ['event', 'time', 'index'])]
