...
def test_get_period():
    ...
    ons2 = xr.DataArray([3.0, 15.5, 25.5])
    dec2 = xr.DataArray([1.5, -7.5, 1.0])
...
def test_flip_cold():
    ds = xr.Dataset()
    y = xr.DataArray([1.0, 2.0, np.nan], dims=["x"], coords=[np.arange(3)])
    z = xr.DataArray([-1.0, -2.0, np.nan], dims=["x"], coords=[np.arange(3)])
